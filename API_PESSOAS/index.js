...
const pessoas = [
    { id: 1, nome: "Evelyn", telefone: "465999756350" }
];
...
app.delete("/pessoas/:id", (request, response) =>{
    ...
    const indexPessoas = pessoa.findIndex(i => i.id == id);

    if (indexPessoas > -1){
        pessoas.splice(indexPessoas, 1);
        return response.send({mansagem : "Pessoa deletada com sucesso!"});
    }else{
        return response.send({mensagem : "O código da pessoa não existe!"});
    }
